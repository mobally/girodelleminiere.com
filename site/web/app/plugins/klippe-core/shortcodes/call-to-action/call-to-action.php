...
class CallToAction implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Call To Action', 'klippe-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'layout',
							'heading'     => esc_html__( 'Layout', 'klippe-core' ),
							'value'       => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Content Elements Proportion', 'klippe-core' ),
							'value'      => array(
								esc_html__( '80/20', 'klippe-core' ) => '80',
								esc_html__( '75/25', 'klippe-core' ) => '75',
								esc_html__( '66/33', 'klippe-core' ) => '66',
								esc_html__( '50/50', 'klippe-core' ) => '50'
							),
							'dependency' => array( 'element' => 'layout', 'value' => array( 'normal' ) )
						),
						array(
						...
						array(
							...
							'heading'    => esc_html__( 'Button Top Margin (px)', 'klippe-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Button Shadow', 'klippe-core' ),
							'value'      => array_flip( klippe_mikado_get_yes_no_select_array( false ) ),
							'group'      => esc_html__( 'Button Style', 'klippe-core' ),
							'save_always'=> true
						),
						...
						array(
							...
							'value'      => esc_html__( 'I am test text for Call to Action shortcode content', 'klippe-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = klippe_core_get_shortcode_module_template_part( 'templates/call-to-action', 'call-to-action', '', $params );
	...
	private function getButtonHolderStyles( $params ) {
		...
		if ( ! empty( $params['button_top_margin'] ) && $params['layout'] === 'simple' ) {
			$styles[] = 'margin-top: ' . klippe_mikado_filter_px( $params['button_top_margin'] ) . 'px';
		}
	...
	private function getButtonclasses( $params ) {
		...
		$classes[] = $params['button_shadow'] === 'yes' && $params['layout'] === 'normal' ? 'enable-shadow' : '' ;
