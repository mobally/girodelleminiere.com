...
class MikadoTwitterWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'mkdf_twitter_widget',
			esc_html__( 'Mikado Twitter Widget', 'klippe-twitter-feed' ),
			array(
				'description' => esc_html__( 'Display your Twitter feed', 'klippe-twitter-feed' )
			)
		);
	...
	private function setParams() {
		$this->params = array(
			array(
				'name'  => 'title',
				'type'  => 'textfield',
				'title' => esc_html__( 'Title', 'klippe-twitter-feed' )
			),
			array(
				'name'    => 'type_of_widget',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Type', 'klippe-twitter-feed' ),
				'options' => array(
					'standard' => esc_html__( 'Standard', 'klippe-twitter-feed' ),
					'slider'   => esc_html__( 'Slider', 'klippe-twitter-feed' )
				)
			),
			array(
				'name'  => 'user_id',
				'type'  => 'textfield',
				'title' => esc_html__( 'User ID', 'klippe-twitter-feed' )
			),
			array(
				'name'  => 'count',
				'type'  => 'textfield',
				'title' => esc_html__( 'Number of Tweets', 'klippe-twitter-feed' )
			),
			array(
				'name'    => 'show_tweet_icon',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Icon', 'klippe-twitter-feed' ),
				'options' => array(
					'yes' => esc_html__( 'Yes', 'klippe-twitter-feed' ),
					'no'  => esc_html__( 'No', 'klippe-twitter-feed' )
				)
			),
			array(
				'name'    => 'show_tweet_time',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Time', 'klippe-twitter-feed' ),
				'options' => array(
					'no'  => esc_html__( 'No', 'klippe-twitter-feed' ),
					'yes' => esc_html__( 'Yes', 'klippe-twitter-feed' )
				)
			...
			array(
				'name'  => 'transient_time',
				'type'  => 'textfield',
				'title' => esc_html__( 'Tweets Cache Time', 'klippe-twitter-feed' )
			)
		);
	}
		...
		if ( $twitter_api->hasUserConnected() ) {
			...
			if ( $response->status ) {
				if ( is_array( $response->data ) && count( $response->data ) ) { ?>
					<ul class="mkdf-twitter-widget <?php echo esc_attr( $holder_classes ); ?>" <?php echo klippe_mikado_get_inline_attrs( $slider_data ); ?>>
						<?php foreach ( $response->data as $tweet ) { ?>
							
							<?php if ( $type_of_widget == 'slider' ) { ?>
								
								<li class="mkdf-tweet-holder">
									<div class="mkdf-tweet-text" <?php klippe_mikado_inline_style( $twitter_style ); ?>>
			...
			esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'klippe-twitter-feed' );
		}
