...
class KlippeMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions         = KlippeMikadoOptions::get_instance();
		$this->mkdMetaBoxes       = KlippeMikadoMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = KlippeMikadoTaxonomyOptions::get_instance();
		$this->mkdUserOptions     = KlippeMikadoUserOptions::get_instance();
		$this->mkdDashboardOptions = KlippeMikadoDashboardOptions::get_instance();
	}
	...
	public function setSkin( KlippeMikadoSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class KlippeMikadoSkinManager
 *
...
class KlippeMikadoSkinManager {
	...
	/**
	 * @see KlippeMikadoSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = MIKADO_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'klippe' );
...
/**
 * Class KlippeMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class KlippeMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( klippe_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo klippe_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'klippe' ) ?>';
		</script>
	<?php }
...
class KlippeMikadoOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $klippe_mikado_options ) ) {
			return $klippe_mikado_options[ $key ];
...
/*
   Class: KlippeMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class KlippeMikadoAdminPage implements iKlippeMikadoLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new KlippeMikadoFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iKlippeMikadoRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: KlippeMikadoMetaBoxes
...
/*
   Class: KlippeMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class KlippeMikadoMetaBox implements iKlippeMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $klippe_mikado_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: KlippeMikadoTaxonomyOptions
   A class that initializes KlippeMikado Taxonomy Options
*/
class KlippeMikadoTaxonomyOptions {
	private static $instance;
...
/*
   Class: KlippeMikadoTaxonomyOption
   A class that initializes KlippeMikado Taxonomy Option
*/
class KlippeMikadoTaxonomyOption implements iKlippeMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new KlippeMikadoTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: KlippeMikadoUserOptions
   A class that initializes KlippeMikado User Options
*/
class KlippeMikadoUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: KlippeMikadoUserOption
   A class that initializes KlippeMikado User Option
*/
class KlippeMikadoUserOption implements iKlippeMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new KlippeMikadoUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: KlippeMikadoDashboardOptions
   A class that initializes KlippeMikado Dashboard Options
*/
class KlippeMikadoDashboardOptions {
	private static $instance;
...
/*
   Class: KlippeMikadoDashboardOption
   A class that initializes KlippeMikado Dashboard Option
*/
class KlippeMikadoDashboardOption implements iKlippeMikadoLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new KlippeMikadoDashboardFieldFactory();
	}
...
if ( ! function_exists( 'klippe_mikado_init_framework_variable' ) ) {
	function klippe_mikado_init_framework_variable() {
		global $klippe_mikado_Framework;
		
		$klippe_mikado_Framework = KlippeMikadoFramework::get_instance();
		$mkdSkinManager                 = new KlippeMikadoSkinManager();
		$klippe_mikado_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'klippe_mikado_before_options_map', 'klippe_mikado_init_framework_variable' );
}
?>