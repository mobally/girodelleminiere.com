...
/*
  Class: KlippeMikadoIconCollections
...
class KlippeMikadoIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new KlippeMikadoIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new KlippeMikadoIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new KlippeMikadoIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new KlippeMikadoLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new KlippeMikadoLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new KlippeMikadoSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new KlippeMikadoDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
	    ...
	    $options = array(
		    'font_awesome'      => esc_html__( 'Font Awesome', 'klippe' ),
		    'font_elegant'      => esc_html__( 'Font Elegant', 'klippe' ),
		    'ion_icons'         => esc_html__( 'Ion Icons', 'klippe' ),
		    'simple_line_icons' => esc_html__( 'Simple Line Icons', 'klippe' )
	    );
	    ...
	    if ( $scope == 'regular' ) {
		    $options = array(
			    ...
			    'font_elegant'      => esc_html__( 'Font Elegant', 'klippe' ),
			    'ion_icons'         => esc_html__( 'Ion Icons', 'klippe' ),
			    'linea_icons'       => esc_html__( 'Linea Icons', 'klippe' ),
			    'linear_icons'      => esc_html__( 'Linear Icons', 'klippe' ),
			    'simple_line_icons' => esc_html__( 'Simple Line Icons', 'klippe' ),
			    'dripicons'         => esc_html__( 'Dripicons', 'klippe' )
		    );
	    }
	    ...
	    foreach ( $icon_collections as $collection_key => $collection_object ) {
		    ...
		    $mkd_icon_pack_container = klippe_mikado_add_admin_container(
			    array(
				    'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'klippe' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'klippe' ),
	...
	public function getIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'klippe' ),
			'options' => array(
				'font_awesome'      => esc_html__( 'Font Awesome', 'klippe' ),
				'font_elegant'      => esc_html__( 'Font Elegant', 'klippe' ),
				'ion_icons'         => esc_html__( 'Ion Icons', 'klippe' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'klippe' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			...
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'klippe' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'klippe' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'klippe' ), $n ),
	...
	public function getIconCollectionIcons( iKlippeMikadoIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'klippe' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'klippe' ) ] = '';
...
if ( ! function_exists( 'klippe_mikado_activate_theme_icons' ) ) {
	function klippe_mikado_activate_theme_icons() {
		global $klippe_mikado_IconCollections;
		$klippe_mikado_IconCollections = KlippeMikadoIconCollections::get_instance();
	}
	
	add_action( 'after_setup_theme', 'klippe_mikado_activate_theme_icons' );
}