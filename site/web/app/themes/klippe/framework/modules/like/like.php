...
class KlippeMikadoLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_klippe_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_klippe_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function like_post( $post_id, $action = 'get', $type = '' ) {
		...
		switch ( $action ) {
			...
			case 'get':
				...
				$return_value = $icon . "<span>" . esc_attr( $like_count ) . esc_html__(' likes', 'klippe') ."</span>";
			...
			case 'update':
				...
				$return_value = "<i class='fa fa-heart' aria-hidden='true'></i><span>" . esc_attr( $like_count ) . esc_html__(' likes', 'klippe') ."</span>";
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'klippe' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'klippe' );
		}
...
if ( ! function_exists( 'klippe_mikado_create_like' ) ) {
	function klippe_mikado_create_like() {
		KlippeMikadoLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'klippe_mikado_create_like' );
}