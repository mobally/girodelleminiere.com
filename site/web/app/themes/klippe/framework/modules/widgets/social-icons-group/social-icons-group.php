...
class KlippeMikadoClassIconsGroupWidget extends KlippeMikadoWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_social_icons_group_widget',
			esc_html__( 'Mikado Social Icons Group Widget', 'klippe' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'klippe' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			klippe_mikado_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'klippe' ),
					'options' => array(
						''             => esc_html__( 'Default', 'klippe' ),
						'square-icons' => esc_html__( 'Square', 'klippe' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Icons Skin', 'klippe' ),
					'description' => esc_html__( 'This applies to the square layout', 'klippe' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'klippe' ),
						'light-skin' => esc_html__( 'Light Skin', 'klippe' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'klippe' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'klippe' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$icon_styles[] = 'font-size: ' . klippe_mikado_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo klippe_mikado_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php klippe_mikado_inline_style( $icon_styles ) ?>
