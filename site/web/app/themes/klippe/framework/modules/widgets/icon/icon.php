...
class KlippeMikadoIconWidget extends KlippeMikadoWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_icon_widget',
			esc_html__( 'Mikado Icon Widget', 'klippe' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'klippe' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			klippe_mikado_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'klippe' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					'type'    => 'dropdown',
					'name'    => 'target',
					'title'   => esc_html__( 'Target', 'klippe' ),
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? klippe_mikado_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$holder_styles[] = 'font-size: ' . klippe_mikado_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		<a <?php klippe_mikado_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo klippe_mikado_get_inline_style( $holder_styles ); ?>>
