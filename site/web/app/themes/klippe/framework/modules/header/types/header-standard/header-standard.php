<?php
namespace KlippeMikado\Modules\Header\Types;

use KlippeMikado\Modules\Header\Lib\HeaderType;
...
class HeaderStandard extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->menuAreaHeight     = klippe_mikado_set_default_menu_height_for_header_types();
			$this->mobileHeaderHeight = klippe_mikado_set_default_mobile_menu_height_for_header_types();
			
			add_action( 'wp', array( $this, 'setHeaderHeightProps' ) );
			
			add_filter( 'klippe_mikado_js_global_variables', array( $this, 'getGlobalJSVariables' ) );
			add_filter( 'klippe_mikado_per_page_js_vars', array( $this, 'getPerPageJSVariables' ) );
		}
	}
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters = apply_filters( 'klippe_mikado_header_standard_parameters', $parameters );
		
		klippe_mikado_get_module_template_part( 'templates/' . $this->slug, $this->moduleName . '/types/' . $this->slug, '', $parameters );
	}
	...
	public function calculateHeightOfTransparency() {
		...
		$menu_background_color             = klippe_mikado_get_meta_field_intersect( 'menu_area_background_color', $id );
		$menu_background_transparency      = klippe_mikado_get_meta_field_intersect( 'menu_area_background_transparency', $id );
		$menu_grid_background_color        = klippe_mikado_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = klippe_mikado_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $menuAreaTransparent ) {
			...
			if ( ( $sliderExists && klippe_mikado_is_top_bar_enabled() )
			     || klippe_mikado_is_top_bar_enabled() && klippe_mikado_is_top_bar_transparent()
			) {
				$transparencyHeight += klippe_mikado_get_top_bar_height();
			}
		}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$menu_background_color             = klippe_mikado_options()->getOptionValue( 'menu_area_background_color' );
		$menu_background_transparency      = klippe_mikado_options()->getOptionValue( 'menu_area_background_transparency' );
		$menu_grid_background_color        = klippe_mikado_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = klippe_mikado_options()->getOptionValue( 'menu_area_grid_background_transparency' );
	...
	public function calculateHeaderHeight() {
		...
		if ( klippe_mikado_is_top_bar_enabled() ) {
			$headerHeight += klippe_mikado_get_top_bar_height();
		}
	...
	public function getPerPageJSVariables( $perPageVars ) {
		...
		$header_behavior = klippe_mikado_get_meta_field_intersect( 'header_behaviour' );
		if ( ! in_array( $header_behavior, array( 'sticky-header-on-scroll-up', 'sticky-header-on-scroll-down-up' ) ) ) {
			$perPageVars['mkdfHeaderTransparencyHeight'] = $this->headerHeight - ( klippe_mikado_get_top_bar_height() + $this->heightOfCompleteTransparency );
		} else {
