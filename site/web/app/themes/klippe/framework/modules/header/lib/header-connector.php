...
namespace KlippeMikado\Modules\Header\Lib;
/**
 * Class KlippeMikadoHeaderConnector
 *
...
class KlippeMikadoHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'klippe_mikado_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'klippe_mikado_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'klippe_mikado_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'klippe_mikado_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to klippe_mikado_title_content_padding filter
	 *
	 * @return int
	 */
