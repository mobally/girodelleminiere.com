...
if ( ! function_exists( 'klippe_mikado_add_custom_nav_fields' ) ) {
	function klippe_mikado_add_custom_nav_fields( $menu_item ) {
	...
	add_filter( 'wp_setup_nav_menu_item', 'klippe_mikado_add_custom_nav_fields' );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'klippe_mikado_update_custom_nav_fields' ) ) {
	function klippe_mikado_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		$check = array(
			'anchor',
			'nolink',
			'hide',
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'klippe_mikado_update_custom_nav_fields', 10, 3 );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'klippe_mikado_edit_walker' ) ) {
	function klippe_mikado_edit_walker( $walker, $menu_id ) {
		return 'KlippeMikado_Walker_Nav_Menu_Edit_Custom';
	}
	
	// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'klippe_mikado_edit_walker', 10, 2 );
}

if ( ! function_exists( 'klippe_mikado_include_custom_walkers' ) ) {
	function klippe_mikado_include_custom_walkers() {
		...
		do_action( 'klippe_mikado_include_custom_walkers_nav' );
	}
	
	add_action( 'after_setup_theme', 'klippe_mikado_include_custom_walkers' );
}